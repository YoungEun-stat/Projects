...
# excel 파일과 rmd 파일이 같은 폴더에 있으면 됩니다.
...
# 모델링2 ('주용도의 세부용도' 변수 제거)
```{r}
new_data_2 <- new_data[-3]
...
# 모델 학습
backward_lm <- lm( `건축공사비/m2` ~ 주건축구분 + 주용도 + 신청기관_구분 + 
    특광역시도 + 부지면적 + 연면적 + 지상층수 + `발주방식(설계)`, 
    data = new_data_2)
summary(backward_lm)
...
# 모델 학습습
forward_lm <- lm(`건축공사비/m2` ~ 연면적 + 주건축구분 + 주용도 + 
    신청기관_구분 + `발주방식(설계)` + 특광역시도 + 지상층수 + 
    부지면적, data = new_data_2)
summary(forward_lm)

## 동일한 결과를 보여줌
## R^2: 0.3962 / Adjusted R^2: 0.3865
...
## oulier가 보임
# 제거하고 refit
lm_final_2 <- lm(`건축공사비/m2` ~ 연면적 + 주건축구분 + 주용도 + 
    신청기관_구분 + `발주방식(설계)` + 특광역시도 + 지상층수 + 
    부지면적, data = new_data_2[-c(1,905,1817, 2445, 2446, 3, 82),])
which.min(fitted(lm_final_2))
summary(lm_final_2)
## R^2: 0.4245 / Adjusted R^2: 0.4151  (R^2 값이 상승함=> 최종 모형으로 선택)
...
ci <- predict(lm_final_2, newdata = data, interval = "confidence", level=0.95)
...
# 데이터에 이상치가 존재해서 축의 범위를 조정해가면서 observed value vs fitted value에 대한 plot을 그렸음
...
for (i in 1:nrow(data_na)){
  if ((data_na[i ,2] %in% unique(new_data_2$`주용도`))==FALSE){
    data_na[i, 2] <- NA
  }
}

# prediction interval 계산
pi <- predict(lm_final_2, newdata = data_na, interval = "prediction", level=0.95)
...
write.csv(final_result, file='분석완료_모델2.csv', row.names=FALSE)
```
